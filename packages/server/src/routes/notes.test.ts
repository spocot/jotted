...
function createApp() {
  ...
  db.exec(`
    ...
    CREATE TABLE note_tags (note_id TEXT NOT NULL REFERENCES notes(id) ON DELETE CASCADE, tag_id TEXT NOT NULL REFERENCES tags(id) ON DELETE CASCADE, source TEXT NOT NULL DEFAULT 'content', PRIMARY KEY (note_id, tag_id));
